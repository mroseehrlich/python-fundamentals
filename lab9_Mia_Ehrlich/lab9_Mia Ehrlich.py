...
# If user chooses option 6, they play Word Guess, which prompts them for a word,
# prints 100 lines, then prompts the user to guess characters for the word. A
# string of asterixes with the length of the word is diplayed, and each correctly
# guessed charchter allows the guess to be printed with the asterix exchanged for
# the charchter in the word. This continues until the user guesses all characters
# correctly.
# If user chooses option 7, the game ends.
...
# a validation loop, then return the menu_option
def display_menu():
    ...
    try:
        ...
        while menu_option < 1 or menu_option > 7:
...
def high_card():
    ...
    # Create two instances of card by calling the card class for
    # card1 and card2
    card1 = card_Ehrlich.Card()
    card2 = card_Ehrlich.Card()

    # Set card 1 and 2 to random ints between 1 and 13 to deal cards
    # by calling deal method
    card1.deal()
    card2.deal()

    # Decision structure for displaying card 1's value
    # Display the name of player one and the value of their card
    print(player1, 'drew a ' + str(card1.get_face_value()) + '.')

    # Decision structre for displaying card 2
    # Display the name of player two and the face value of their card
    print(player2, 'drew a ' + str(card2.get_face_value()) + '.')
...
# Inputs: card_list
# Outputs: none    
# Purpose: The display_hand() function loops through a five item list of card objects
# and displays the face value of each individual card by passing the get_face_value()
# method to the print function
def display_hand(card_list):
    #Display card values in card list
    for card in card_list:
        print(card.get_face_value())

# Function: hand_stats()
# Inputs: card_list
# Outputs: none
# Purpose: The hand_stats() sets an accumulator to 0, loops through each value data
# attribute for each object in the card_list, then adds each value to the total. It
# then computes the average card value by deviding the total by the number of cards
# in the list and prints both the total card valiue and the average card value.
...
# Inputs: none
# Outputs: none
# Purpose: The deal_hand() function creates a five item list of cards objects
#  with values of random integers between 1 and 13 by calling the deal() method
# It then displays the face value of each card in the list as well as their total
# combined total value and average value.
def deal_hand():
    ...
    # Create 5 object instances of the Card class and assign random integers
    # Between 1 and 13 by calling deal method add cards to list
    
    for card in range(5):
        card = card_Ehrlich.Card()
        card.deal()
...
# Inputs: none
# Outputs: none
# Purpose: The save_dream_hand() function prompts the user for card values with
# numbers between 1-13, validates the input, and then creates five card objects,
# sets their value to the user inpute value, and appends each card to a list. It
# then prompts the user to enter a file name and writes the five cards to the file
# as list, alerting the user that the file was saved. If a value error is raised
# due to the user entering a non-integer value, an except suit runs to notify the
# user of their error.
def save_dream_hand():
    ...
    try:
        ...
        for card in range(5):
            # Set card value for card
            card_value = int(input('Please enter a number for your dream card: '))
            
            # Validate user input for each of the 5 cards
            while card_value < 1 or card_value > 13:
                print('Invalid option.')
                card_value = int(input('Please enter a number for your dream card: '))
...
# room, prompts the user for an existing file name, opens the file, creates
# objects with values referncing those from the list, closes the file, and displays
...
def display_dream_hand():
    ...
    try:
        ...
        # Read card values from file, create card instances, set value to int
        # value of line in file, append to card_list
        for line in infile:
            card_value = int(line)
            card = card_Ehrlich.Card()
            card.set_value(int(line))
            card_list.append(card)
...
# Inputs: none
# Outputs: none
# Purpose: The word_guess() function displays an introduction to the word guess
# gaming room, promts the user for a word, and then prints 100 new lines. It then
# sets a variable for a string of letters guessed and a guess of a number of '*"
# equal to the length of the word. While the user has not guessed the word, the user
# is prompted to guess a character and are alerted if they have already guessed that
# character. The guessof asterixes is then displayed. If the guessed character is correct,
# the guess is displayed with each asterix at every index of the character in the word is
# replaced by the guessed character. Once the user has guessed all characters correctly,
# the program displays the word and that they won the game. 
def word_guess():
    # Introduce word guess game
    print('\nWORD GUESS\n')
    
    word = input('Enter a word: ')
    # Print 100 new lines and invite user to guess word
    print('\n' * 100)
    print('\nLet\'s guess your word!\n')

    # Create string of letters already guessed
    letters_guessed = "";
    # Set guess to a series of asterix with the length of the word
    guess = ('*' * len(word))

    # Set while loop to ask user to guess charchters until guess = word
    while guess != word:
        
        char_guessed = input('\nEnter a letter: ')

        # Alert user that charcter was already guessed
        if char_guessed in letters_guessed:
            print('You have already guessed ' + '\'' + char_guessed + '\'.' )
        else:

            # Create copy of guess to track progress of guessed repeat characters
            # at multiple indexes
            guess_progress = word
            for ch in guess:
                if char_guessed in guess_progress:
                    # Find first index of character
                    position = guess_progress.find(char_guessed)
                    # Add character to guess string at position index
                    guess = guess[:position] + char_guessed + guess[position + 1:]
                    # Replace character in guess_progress with '*' so the next loop
                    # Finds the next index of repeat charcters
                    guess_progress = guess_progress[:position] + '*' + guess_progress[position+1:]        
            # Add char_guessed to letters_guessed
